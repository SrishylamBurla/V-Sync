...
export default function DashboardPage() {
  ...
  return (
    <div className="space-y-6 py-5 sm:py-7">
      <section className="rounded-3xl border border-blue-100 bg-gradient-to-br from-blue-50 via-white to-violet-50 p-5 shadow-sm sm:p-6 lg:p-7">
      ...
      <section className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_minmax(520px,600px)] xl:items-start">
      ...
      <aside className="sticky top-5 h-fit max-h-[calc(100vh-2.5rem)] overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-[0_16px_40px_rgba(15,23,42,0.08)]">
        <div className="border-b border-slate-200 p-5 sm:p-6">
          ...
          <div className="mt-4 grid gap-2 xl:grid-cols-[minmax(0,1fr)_140px_150px]">
        ...
        <div className="max-h-[calc(100vh-370px)] overflow-y-auto overflow-x-auto overscroll-contain">
          {loading && (
...
function WorkspaceSection({ user, navigate, isAdmin }) {
  const role = user?.role;

  const can = (module) => canAccessModule(module, role);

  const groups = [
    {
      key: "patients",
      title: "Patients & Care",
      description: "Patient records, appointments and follow-up communication.",
      icon: UserRound,
      tone: "blue",
      items: [
        { label: "Patients", description: "Patient list and complete records", to: "/patients", icon: Users, access: "patients" },
        { label: "Appointments", description: "Book, manage and reschedule", to: "/appointments", icon: CalendarClock, access: "appointments" },
        { label: "Clinical", description: "Consultations and prescriptions", to: "/clinical", icon: Stethoscope, access: "clinical" },
        { label: "Recall list", description: "Patients due for follow-up", to: "/recall", icon: History, access: "recall" },
        { label: "SMS / Email", description: "Patient communications", to: "/communications", icon: BellIcon, access: "communications" },
      ],
    },
    {
      key: "financial",
      title: "Financial",
      description: "Billing, outstanding amounts and financial reporting.",
      icon: WalletCards,
      tone: "emerald",
      items: [
        { label: "Billing", description: "Create and manage invoices", to: "/billing", icon: FileText, access: "billing" },
        { label: "Finance", description: "Expenses, petty cash and transactions", to: "/finance", icon: WalletCards, access: "finance" },
        { label: "Reports", description: "Business activity and financial reports", to: "/reports", icon: FileBarChart, access: "reports" },
      ],
    },
    {
      key: "inventory",
      ...
      items: [
        ...
        { label: "Laboratory", description: "Lab jobs and production flow", to: "/lab", icon: Activity, access: "laboratory" },
      ],
    },
    {
      key: "maintenance",
      title: "Maintenance & Setup",
      description: "People, branches and application configuration.",
      icon: Wrench,
      tone: "amber",
      items: [
        { label: "Staff", description: "Users, roles and staff records", to: "/staff", icon: Users, access: "staff" },
        { label: "Branches", description: "Branch details and configuration", to: "/branches", icon: Building2, access: "branches" },
        { label: "Settings", description: "Practice and application settings", to: "/settings", icon: Settings, access: "settings" },
        ...(isAdmin
          ? [{ label: "Organizations", description: "Organization administration", to: "/admin/organizations", icon: Building2, access: "organizations" }]
          : []),
      ],
    },
  ];
  ...
  const toneMap = {
    ...
    violet: {
      header: "border-violet-100 bg-violet-50/70",
  ...
  return (
    <section className="space-y-4">
      <div className="flex flex-col gap-1 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="mt-1 max-w-3xl text-xs leading-5 text-slate-500">
            Follow the workflow from patient registration to clinical care, optical orders,
            inventory, billing and practice setup without hunting through unrelated menus.
          </p>
        </div>
        <div className="rounded-full border border-slate-200 bg-white px-3 py-1.5 text-[9px] font-bold uppercase tracking-[.16em] text-slate-400">
      ...
      <div className="grid gap-4 xl:grid-cols-2">
        {groups.map((group) => {
          const visibleItems = group.items.filter((item) => can(item.access));
          ...
          return (
            ...
            >
              <div className={`border-b px-4 py-4 ${tone.header}`}>
                <div className="flex items-start gap-3">
                  <span className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-xl shadow-sm ${tone.icon}`}>
                ...
                {visibleItems.map((item) => {
                  ...
                  return (
                    <button
                      ...
                      className={`group flex min-h-[66px] items-center gap-3 rounded-xl border border-slate-100 bg-white px-3 py-2.5 text-left transition ${tone.hover}`}
                    >
                      <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-slate-100 text-slate-500 transition group-hover:bg-white group-hover:text-slate-800">
                        <Icon size={15} />
                      </span>
...
//     } catch (e) {
//       setError(
//         e?.response?.data?.message || "Unable to load today's appointments"
//       );
//     } finally {
//       setLoading(false);
//     }
...
//           status: "active",
//         });
//         setPatientResults(normalizePatients(response));
//       } catch {
//         setPatientResults([]);
//       } finally {
...
//         ["booked", "confirmed", "here"].includes(a.status)
//       ).length,
//       examining: appointments.filter((a) => a.status === "examining").length,
...
//     if (!form.patientId) {
//       setError("Please select a patient before creating the appointment.");
//       return;
//     }
...
//     } catch (e) {
//       setError(
//         e?.response?.data?.message || "Unable to create appointment"
//       );
//     }
//   };
...
//     (appointment) =>
//       !["complete", "cancelled", "no_show"].includes(appointment.status)
//   );
...
//   return (
//     <div className="min-h-full bg-slate-50/50 py-5 sm:py-7">
//       <div className="mx-auto max-w-[1600px] space-y-5">
//         <section className="relative overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
//           <div className="absolute inset-0 bg-[radial-gradient(circle_at_top_right,_rgba(15,23,42,0.06),_transparent_42%)]" />
//           <div className="relative flex flex-col gap-5 px-5 py-5 sm:px-7 sm:py-6 xl:flex-row xl:items-center xl:justify-between">
//             <div>
...
//                 type="button"
//                 onClick={loadAppointments}
//                 className="inline-flex h-10 items-center gap-2 rounded-xl border border-slate-200 bg-white px-3.5 text-xs font-semibold text-slate-600 transition hover:border-slate-300 hover:bg-slate-50"
//               >
//                 <Clock3 size={14} />
//                 Refresh
//               </button>
...
//                 className="inline-flex h-10 items-center gap-2 rounded-xl border border-slate-200 bg-white px-3.5 text-xs font-semibold text-slate-700 transition hover:border-slate-300 hover:bg-slate-50"
//               >
//                 <Users size={14} />
//                 Find patient
//               </button>
...
//         {error && (
//           <div className="flex items-start justify-between gap-4 rounded-2xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700 shadow-sm">
//             <span>{error}</span>
//             <button
//               type="button"
//               onClick={() => setError("")}
...
//             note={`${completionRate}% of today's visits`}
//           />
//           <Metric
//             icon={XCircle}
//             label="Cancelled / no-show"
...
//                   </div>
//                   <p className="mt-1 text-xs text-slate-400">
//                     Keep patient flow moving from one schedule to the next.
//                   </p>
//                 </div>
...
//                   type="button"
//                   onClick={() => setShowPatientList(true)}
//                   className="inline-flex items-center gap-2 self-start rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-xs font-semibold text-slate-600 transition hover:bg-white"
//                 >
//                   Find patient
//                   <Search size={13} />
//                 </button>
...
//                 <table className="w-full min-w-[980px] text-left">
//                   <thead>
//                     <tr className="border-b border-slate-100 bg-slate-50/80 text-[10px] uppercase tracking-[0.14em] text-slate-400">
//                       <th className="px-5 py-3">Time</th>
//                       <th className="px-5 py-3">Patient</th>
...
//                           <div className="text-sm font-bold text-slate-800">
//                             {fmtTime(appointment.appointmentDate)}
//                           </div>
//                           <div className="mt-1 text-[10px] text-slate-400">
...
//                         <td className="px-5 py-4">
//                           <div className="text-sm text-slate-700">
//                             {fullName(appointment.clinicianId) || "Unassigned"}
//                           </div>
//                           <div className="mt-1 text-[10px] text-slate-400">
//                             {appointment.clinicianId?.role || ""}
//                           </div>
//                         </td>
...
//                     type="button"
//                     onClick={() => setShowAppointmentForm(true)}
//                     className="inline-flex h-10 items-center justify-center gap-2 rounded-xl bg-white px-3 text-xs font-bold text-slate-950 transition hover:bg-slate-100"
//                   >
//                     <Plus size={14} />
//                     Add visit
//                   </button>
...
//                     type="button"
//                     onClick={() => setShowPatientList(true)}
//                     className="inline-flex h-10 items-center justify-center gap-2 rounded-xl border border-white/15 bg-white/5 px-3 text-xs font-bold text-white transition hover:bg-white/10"
//                   >
//                     <Search size={14} />
//                     Patient
//                   </button>
...
//                     type="button"
//                     onClick={() =>
//                       navigate(`/patients/${nextAppointment.patientId?._id}`)
//                     }
//                     className="mt-3 flex w-full items-center justify-between gap-3 text-left"
//                   >
//                     <div className="min-w-0">
//                       <div className="text-sm font-semibold text-white">
//                         {fullName(nextAppointment.patientId) || "Unknown patient"}
//                       </div>
//                       <div className="mt-1 text-xs text-slate-400">
//                         {fmtTime(nextAppointment.appointmentDate)} ·{" "}
//                         {nextAppointment.type || "Eye Examination"}
//                       </div>
//                     </div>
...
//                     Open the complete patient record or start a new visit.
//                   </p>
//                 </div>
...
//                 type="button"
//                 onClick={() => setShowPatientList(true)}
//                 className="mt-5 flex w-full items-center justify-between rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-left text-xs font-semibold text-slate-700 transition hover:border-slate-300 hover:bg-white"
//               >
...
//                 </h2>
//                 <p className="mt-1 text-xs text-slate-400">
//                   Search and open the complete patient record.
//                 </p>
//               </div>
//               <button
//                 type="button"
...
//                   className="h-12 w-full rounded-xl border border-slate-200 bg-slate-50 pl-10 pr-4 text-sm text-slate-700 outline-none transition focus:border-slate-400 focus:bg-white focus:ring-2 focus:ring-slate-100"
//                 />
//               </div>
//             </div>
...
//                   <div className="hidden border-b border-slate-200 bg-slate-50 px-4 py-3 text-[10px] font-bold uppercase tracking-wider text-slate-400 sm:grid sm:grid-cols-[minmax(0,1.4fr)_140px_150px_100px]">
//                     <span>Patient</span>
...
//                     <div
//                       key={patient._id}
//                       className="grid gap-3 border-b border-slate-100 px-4 py-4 last:border-b-0 hover:bg-slate-50/60 sm:grid-cols-[minmax(0,1.4fr)_140px_150px_100px] sm:items-center"
//                     >
//                       <button
//                         type="button"
//                         onClick={() =>
...
//                         <span className="mr-2 text-[10px] uppercase tracking-wider text-slate-400 sm:hidden">
//                           ID
//                         </span>
...
//             <form onSubmit={saveAppointment} className="p-5 sm:p-6">
...
//                       <input
//                         value={patientQuery}
//                         readOnly={Boolean(form.patientId)}
//                         placeholder="Select a patient"
//                         className="h-11 min-w-0 flex-1 rounded-xl border border-slate-200 bg-slate-50 px-3 text-sm text-slate-700 outline-none focus:bg-white"
//                         required
//                       />
//                       <button
//                         type="button"
...
//                       }))
//                     }
//                     className="h-11 w-full rounded-xl border border-slate-200 bg-slate-50 px-3 text-sm text-slate-700 outline-none focus:bg-white"
//                   >
...
//                     }
//                     rows={4}
//                     className="w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-3 text-sm text-slate-700 outline-none focus:bg-white"
...
// function Metric({ icon: Icon, label, value, note, emphasis = false }) {
//   return (
//     <div
